...
public class InfectedPlayerUpdateEvent /* extends EntityDragon */{
	...
	@SuppressWarnings("rawtypes") @SubscribeEvent public void onPlayerUpdate(
		...
		if (player.isPotionActive(ZombieInfection.potionInfection) && !player.isPotionActive(ZombieInfection.potionCure)) {
			...
			for (CustomInfectionEffect customEffect : ZombieInfectionAPI.getCustomInfectionEffects()) {customEffect.run(player, timeInfected);
			}
			player.addPotionEffect(PotionHelper
					.createInfection(timeInfected < (20 * 60 * 2) ? 0 : 1));
			player.addPotionEffect(PotionHelper
					.createHunger(timeInfected < (20 * 60 * 2) ? 0 : 1));
			player.addPotionEffect(PotionHelper
					.createSlowness(timeInfected < (20 * 60 * 2) ? 0 : 1));
			player.addPotionEffect(PotionHelper
					.createMiningFatigue(timeInfected < (20 * 60 * 2) ? 0 : 1));
			player.addPotionEffect(PotionHelper
					.createWeakness(timeInfected < (20 * 60 * 2) ? 0 : 1));
			if (player.getFoodStats().getFoodLevel() > FoodTracking.get(player)) {
				player.getFoodStats().addStats(
						FoodTracking.get(player)
			...
			if (FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER) {
				...
				if (player.worldObj.canBlockSeeTheSky((int) player.posX,
